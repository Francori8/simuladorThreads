...
const limiteDeRepeticionesActual = () => {
  return $("input[name='limite']:checked").value;
};
...
function agregarTab(e) {
  ...
  if (e.keyCode === tabKey) {
    ...
    textArea.value =
      valorAnterior.substring(0, inicio) +
      "\t" +
      valorAnterior.substring(final);
...
const averiguarProbabilidad = () => {
  return $("#porcentaje").value * (100 / $("#porcentaje").max / 100);
};
...
function crearThreads(arrayTexto, mem) {
  ...
  arrayTexto.forEach((value) => {
    const num = parseInt(
      value[0].substring(6).replace("(", "").replace(")", "")
    );
    for (let index = 0; index < num; index++) {
      arrayTextConRep.push(value);
    }
  });
  return arrayTextConRep.map(
    (value) =>
      new Hilo(
        idThread++,
        new Memoria(),
        mem,
        crearInstruccionCon(value.toSpliced(0, 1), mem)
      )
  );
}
...
function establecerMemoria(arraysDeGlobal, mem) {
  arraysDeGlobal.forEach((string) => manejarMemoria(string.substring(6), mem));
}
...
function crearInstruccionCon(instruccionesString, mem) {
  return instruccionesString.map((string) =>
    instruccionSegunString(string, mem)
  );
}
...
function instruccionSegunString(string, mem) {
  ...
  if (string.startsWith("while")) {
    const stringAResolver = string
      .substring(5)
      .replace("(", "")
      .replace(")", "")
      .replace("{", "");
    const limiteDeRepeticiones = limiteDeRepeticionesActual();
    return new While(
      instruccionSegunString(stringAResolver, mem),
      limiteDeRepeticiones
    );
  }
  ...
  if (tieneEscritura(string)) {
    ...
    return new Escritura(
      variableAEscribir,
      instruccionSegunString(asignacion.join("="), mem)
    );
  }
  ...
  if (string.includes("&&")) {
    ...
    return new YLogico(
      instruccionSegunString(primeraCondicion, mem),
      instruccionSegunString(condicones.join("&&"), mem)
    );
  }
  if (string.includes("||")) {
    const condicones = string.split("||");
  ...
  if (string.includes(">=")) {
    ...
    return new MayorOIgual(
      instruccionSegunString(variableAEscribir, mem),
      instruccionSegunString(asignacion.join(""), mem)
    );
  }
  if (string.includes("<=")) {
  ...
  if (string.includes("==")) {
    ...
    return new Igualdad(
      instruccionSegunString(variableAEscribir, mem),
      instruccionSegunString(asignacion[0], mem)
    );
  }
  if (string.includes("+")) {
    ...
    return new Sumar(
      instruccionSegunString(restoC, mem),
      instruccionSegunString(instruccion.join("+"), mem)
    );
  }
...
function crearBotones(contenedor, elementos, funcionOnClick) {
  const botonesHTML = elementos.map((val) => {
    return `<button data-btnid=${val.id} title="${val.razon}" class="ejemplo-btn"> ${val.id}</button>`;
  });
